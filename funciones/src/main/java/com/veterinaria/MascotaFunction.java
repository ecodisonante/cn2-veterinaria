...
public class MascotaFunction {
    private static final MascotaService srv = new MascotaService();
    private static final ObjectMapper om = new ObjectMapper();

    @FunctionName("Mascota")
    public HttpResponseMessage handle(
            @HttpTrigger(name = "req", methods = { HttpMethod.GET, HttpMethod.POST, HttpMethod.PUT,
                    HttpMethod.DELETE }, route = "mascota/{id?}", authLevel = AuthorizationLevel.ANONYMOUS) HttpRequestMessage<Optional<String>> req,
    ...
    private HttpResponseMessage handleGet(Long id, HttpRequestMessage<Optional<String>> req) throws SQLException {
        if (id != null) {
            ...
            if (result != null) {
                ...
                return HttpUtils.notFoundMessage(req, "Mascota");
    ...
    private HttpResponseMessage handleUpdate(Long id, HttpRequestMessage<Optional<String>> req)
            throws JsonProcessingException, SQLException {
        if (id == null) {
            return HttpUtils.badRequestMessage(req, "El ID es requerido para actualizar una mascota.");
        }
    ...
    private HttpResponseMessage handleDelete(Long id, HttpRequestMessage<Optional<String>> req) throws SQLException {
        if (id == null) {
            return HttpUtils.badRequestMessage(req, "El ID es requerido para eliminar una mascota.");
        }
